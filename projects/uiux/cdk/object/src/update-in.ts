...
import { getIndexAsNumber, isArraySyntax } from './key-converter';
import { default as isArray } from 'lodash-es/isArray';
import { splitKeysIntoDotNotation } from './key-converter';
/**
 * @license
 * Copyright UIUX Engineering All Rights Reserved.
 */
...
export function updateIn(object: any, keys: string | string[], value: any) {
  ...
  if (isArray(object) && isArraySyntax(keys[0])) {
    ...
    // if object exists in array
    if (hasValue(object[index])) {
      // Are we drilling to object in array
      if (keys[1]) {
